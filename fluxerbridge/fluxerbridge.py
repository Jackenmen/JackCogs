...
class InstanceEmojiCache:
    ...
    @classmethod
    def from_dict(cls, cog: FluxerBridge, hostname: str, data: Dict[str, Any]) -> Self:
        return cls(
            cog,
            hostname,
    ...
    def _emoji_uploader(
        ...
        [discord.PartialEmoji], Coroutine[None, None, Optional[discord.PartialEmoji]]
    ]:
        uploaded_count = 0
        allow_uploads = True

        async def upload(emoji: discord.PartialEmoji) -> Optional[discord.PartialEmoji]:
...
class FluxerBridge(commands.Cog):
    ...
    async def fluxerbridge_emojis_configure(
        ...
        emoji_cache = InstanceEmojiCache(
            self, api_hostname, guild_id=guild_id, token=msg.content
        )
    ...
    async def fluxerbridge_emojis_maxuploadlimit(
        self, ctx: commands.GuildContext, *, value: commands.Range[int, 0, 50] = None
    ) -> None:
        """
        ...
        await ctx.send(f"The new per-messeage upload limit has been set to {value}.")
