    ...
    async def convert(
        ...
        if not maybe_matches:
            raise commands.BadArgument((
                '"{arg}" was not found. It must be the ID, mention, '
...
class MemberOrRoleorVoiceChannel(commands.Converter):
    async def convert(
        ...
        objects = itertools.chain(
            guild.voice_channels, guild.members, filter(lambda r: not r.is_default(), guild.roles)
        )
        ...
        if not maybe_matches:
            ...
            raise commands.BadArgument((
                    '"{arg}" does not refer to a unique channel, user or role. '
